...
void printFAT(){
        for(int i = 0; i < MAX_FILES; i++){
                struct FAT current = fat[i];
                printf("filename = %s, file length = %d, read = %d, write = %d\n", current.filename, current.file_length, current.read, current.write);
        }
}
...
int mountFS(char *name){
    ...
    for (int i = 0; i < MAX_FILES; i++) {
        ...
        currentFile.file_length = atoi(token);
       	...
       	token = strtok(NULL, &comma);
	printf("read = %s\n", token);
	currentFile.read = atoi(token);

	token = strtok(NULL, &comma);
	printf("write = %s\n", token);
	currentFile.write = atoi(token);
...
void updateFATOnDisk(){
    ...
    for (int i = 0; i < MAX_FILES; i++) {
        ...
        sprintf(writebuffer2, "%s, %i, %i, %i\n %s\n", currentFile.filename, currentFile.file_length, currentFile.read, currentFile.write, writebuffer);
