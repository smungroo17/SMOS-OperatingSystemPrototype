...
int chmodCommandEXAM(char *parsedCommand[]);
...
int interpreter(char *parsedCommand[]){
    ...
    else if(strcmp(&parsedCommand[0][0], "chmod_EXAM") == 0)
	    return chmodCommandEXAM(parsedCommand);
...
int writeCommandEXAM(char *parsedCommand[]){
	if(parsedCommand[1] == NULL || parsedCommand[2] == NULL){
                printf("The write command requires a file_id in the active table to write to as well as a data to write\n");
                return 1;
	}
	
	int k = 0;
        char c = *(parsedCommand[1] + k);
        for(; k < strlen(parsedCommand[1]); k++){
                if(isdigit(c) == 0){
                        printf("The first argument needs to be an integer\n");
                        return 1;
                }
                c = *(parsedCommand[1]+k);
	...
	float length = strlen(parsedCommand[2]);
        float blockSize = getBlockSize();

        for (int i = 0; i < (length/blockSize)+1; i++) {
                int put = i * getBlockSize();
                char *curr = (parsedCommand[2] + put);
                int result = write_EXAM(atoi(parsedCommand[1]), curr);
                if(result == -1){
...
int readCommandEXAM(char *parsedCommand[]){
    ...
    // DISK_driver_problem1.c [.h] that were provided to you
        if(parsedCommand[1] == NULL || parsedCommand[2] == NULL){
                printf("The read command requires a file_id in the active table to read from as well as a variable name to read into\n");
                return 1;
        }
        ...
        int j;
        for(j = 0; j < 10; j++){
                if(fat[active_file_table_owners[atoi(parsedCommand[1])]].blockPtrs[j] == -1){
                        break;
                }
                num_blocks++;
        }
        int count;
        //check if we are at the beginning of file
        //SEEK should work fine
        if(fat[active_file_table_owners[atoi(parsedCommand[1])]].current_location == 0){
                count = 0;
        }
        //get the number of blocks between current location and end of file
        else{
                count = fat[active_file_table_owners[atoi(parsedCommand[1])]].current_location - 1;
        }
        while(ptr != NULL && count < num_blocks-1){
                ptr = read_EXAM(atoi(parsedCommand[1]), parsedCommand[2], count);
		if (ptr == NULL){
                        break;
                }
                fat[active_file_table_owners[atoi(parsedCommand[1])]].current_location += 1;
                count += 1;
                strcat(finalStr, ptr);
        }
    // Write the string to the shell variable
...
int openCommandEXAM(char *parsedCommand[]){
    ...
    // DISK_driver_problem1.c [.h] that were provided to you
        if (parsedCommand[1] == NULL || parsedCommand[2] == NULL ){
                printf("The open command requires an active file table index and a filename as input\n");
                return 1;
        }
	...
	open_EXAM(atoi(parsedCommand[1]), parsedCommand[2]);
    // You can make visible any data structures that you 
    // might need, by importing the appropriate headers
    // and making needed data structures visible.
    return 0;
...
int closeCommandEXAM(char *parsedCommand[]){
	...
	if (parsedCommand[1] == NULL){
                printf("The close command requires an active file table index passed as input\n");
		return 1;
        }
	...
	close_EXAM(atoi(parsedCommand[1]));	
    // You can make visible any data structures that you 
    // might need, by importing the appropriate headers
    // and making needed data structures visible.
    return 0;
...
int seekCommandEXAM(char *parsedCommand[]){
    ...
    // DISK_driver_problem3.c [.h] that were provided to you
   	if (parsedCommand[1] == NULL || parsedCommand[2] == NULL){
                printf("The seek command requires an active file table index and an offset\n");
		return 1;
        }
	...
	int k = 0;
        ...
        k = 0;
        c = *(parsedCommand[2] + k);
        for(; k < strlen(parsedCommand[2]); k++){
                if(isdigit(c) == 0 && c != '-'){
                        printf("The second argument needs to be an integer\n");
                        return 1;
                }
                c = *(parsedCommand[2]+k);
	...
	seek_EXAM(atoi(parsedCommand[1]), atoi(parsedCommand[2]));
...
int chmodCommandEXAM(char *parsedCommand[]){
	if(parsedCommand[1] == NULL){
		printf("The chmod command requires an active file table index.\n");
		return 1;
	}
	else if(parsedCommand[2] == NULL || parsedCommand[3] == NULL){
		printf("ERROR: Bad Syntax\n");
		return -2;
	}
	else if((strcmp(parsedCommand[2], "r+") != 0 && strcmp(parsedCommand[2], "r-") != 0 && strcmp(parsedCommand[2], "w+") != 0 &&
			strcmp(parsedCommand[2], "w-") != 0) || (strcmp(parsedCommand[3], "r+") != 0 && strcmp(parsedCommand[3], "r-") != 0 && 
			strcmp(parsedCommand[3], "w+") != 0 &&strcmp(parsedCommand[3], "w-") != 0)){
		printf("ERROR: Bad Syntax\n");
		return 1;
	}
	int k = 0;
	...
	if(strcmp(parsedCommand[2], "r+") == 0){
		readP = 1;
	}
	if(strcmp(parsedCommand[3], "w+") == 0){
                writeP = 1;
        }
	chmod_EXAM(atoi(parsedCommand[1]), readP, writeP);
	return 0;
}
